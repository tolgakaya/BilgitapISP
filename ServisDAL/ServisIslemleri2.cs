...
namespace ServisDAL
{
    ...
    public class ServisIslemleri2
    {

        radiusEntities db;

        public ServisIslemleri2(radiusEntities db)
        {
        ...
        public List<ServisRepo> servisHepsiAcikDetayR()
        {
            return (from s in db.services
                    where s.KapanmaZamani == null && s.iptal == false
        ...
        //kapanmamış tamirciservisleri
        //ayrıca kapanmışları da gösterelim.
        //kapanmış servislerde ödeme yapılabilsin.
        //kapanmamışlarda kapatabilsin.
        ...
        public sayfali servisSayfaliTest(bool kapanma, int sayfaNo, int perpage, DateTime? zaman = null)
        {
        ...
        public List<ServisRepo> servisHepsiAcikDetayRTip(string tipID)
        {
            int tip = Int32.Parse(tipID);
            return (from s in db.services
                    where s.KapanmaZamani == null && s.iptal == false && s.tip_id == tip
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
                        serviceID = s.ServiceID,
        ...
        public List<ServisRepo> servisHepsiDetayMR(int id)
        {
            return (from s in db.services
                    where s.iptal == false && s.CustID == id
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
                        serviceID = s.ServiceID,
        ...
        public List<ServisRepo> servisHepsiDetayMRTip(int id, string tipID)
        {
            int tip = Int32.Parse(tipID);
            return (from s in db.services
                    where s.iptal == false && s.CustID == id && s.tip_id == tip
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
                        serviceID = s.ServiceID,
        ...
        public List<ServisRepo> servisUrun(string urun_kimlik)
        {
            IEnumerable<ServisRepo> direkt = (from s in db.services
                                              where s.iptal == false && s.urun.BelgeYol == urun_kimlik
            ...
            IEnumerable<ServisRepo> dolayli = (from s in db.services
                                               ...
                                               where s.iptal == false && s.urun.BelgeYol != urun_kimlik && h.ServiceID == s.ServiceID && h.Urun_Kimlik == urun_kimlik
        ...
        public List<ServisRepo> servisUrunTip(string urun_kimlik, string tipID)
        {
            int tip = Int32.Parse(tipID);
            IEnumerable<ServisRepo> direkt = (from s in db.services
                                              where s.iptal == false && s.urun.BelgeYol == urun_kimlik && s.tip_id == tip
            ...
            IEnumerable<ServisRepo> dolayli = (from s in db.services
                                               from h in db.servicehesaps
                                               where s.iptal == false && s.tip_id == tip && s.urun.BelgeYol != urun_kimlik && h.ServiceID == s.ServiceID && h.Urun_Kimlik == urun_kimlik
        ...
        public List<ServisRepo> servisHepsiAcikDetayGorevliR(string gorevliID)
        {
            return (from s in db.services
                    where s.KapanmaZamani == null && s.iptal == false && (s.SonAtananID == gorevliID || s.SonAtananID.Equals("0"))
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
                        serviceID = s.ServiceID,
                        ...
                        kapanmaZamani = s.KapanmaZamani,
                        urunID = s.UrunID,
                        urunAdi = s.UrunID == null ? "HİZMET" : s.urun.Cinsi,
                        kimlikNo = s.Servis_Kimlik_No,
                        tipID = s.tip_id,
                        css = s.service_tips.css,
                        servisTipi = s.service_tips.tip_ad,
        ...
        public List<ServisRepo> servisHepsiAcikDetayGorevliRTip(string gorevliID, string tipID)
        {
            int tip = Int32.Parse(tipID);

            return (from s in db.services
                    where s.KapanmaZamani == null && s.iptal == false && s.tip_id == tip && (s.SonAtananID == gorevliID || s.SonAtananID.Equals("0"))
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
                        serviceID = s.ServiceID,
        ...
        public List<ServisRepo> servisHepsiDetayGorevliR(string gorevliID, int id)
        {
            return (from s in db.services
                    where s.iptal == false && (s.SonAtananID == gorevliID || s.SonAtananID.Equals("0")) && s.CustID == id
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
                        serviceID = s.ServiceID,
        ...
        public List<ServisRepo> servisHepsiDetayGorevliRTip(string gorevliID, int id, string tipID)
        {
            int tip = Int32.Parse(tipID);

            return (from s in db.services
                    where s.iptal == false && (s.SonAtananID == gorevliID || s.SonAtananID.Equals("0")) && s.CustID == id && s.tip_id == tip
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
                        serviceID = s.ServiceID,
        ...
        //manager için
        public List<ServisRepo> servisAraAcikR(string kelime)
        {
            return (from s in db.services
                    where s.KapanmaZamani == null && s.iptal == false && (s.customer.Ad.Contains(kelime) || s.Baslik.Contains(kelime) || s.urun.Cinsi.Contains(kelime) || s.urun.imeino.Contains(kelime) || s.urun.serino.Contains(kelime) || s.urun.digerno.Contains(kelime) ||
                    s.Servis_Kimlik_No.Contains(kelime) || s.Aciklama.Contains(kelime) || s.customer.TC.Contains(kelime) || s.customer.tanimlayici.Contains(kelime) || s.customer.Adres.Contains(kelime))
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
                        serviceID = s.ServiceID,
        ...
        public List<ServisRepo> servisAraAcikRTip(string kelime, string tipID)
        {
            int tip = Int32.Parse(tipID);
            return (from s in db.services
                    where s.KapanmaZamani == null && s.iptal == false && s.tip_id == tip && (s.customer.Ad.Contains(kelime) || s.Baslik.Contains(kelime) || s.urun.Cinsi.Contains(kelime) || s.urun.imeino.Contains(kelime) || s.urun.serino.Contains(kelime) || s.urun.digerno.Contains(kelime) ||
                    s.Servis_Kimlik_No.Contains(kelime) || s.Aciklama.Contains(kelime) || s.customer.TC.Contains(kelime) || s.customer.tanimlayici.Contains(kelime) || s.customer.Adres.Contains(kelime))
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
        ...
        public List<ServisRepo> servisAraMR(string kelime, int custID)
        {
            return (from s in db.services
                    where s.iptal == false && s.CustID == custID && (s.Baslik.Contains(kelime) || s.urun.Cinsi.Contains(kelime) || s.urun.imeino.Contains(kelime) || s.urun.serino.Contains(kelime) || s.urun.digerno.Contains(kelime) ||
                    s.Servis_Kimlik_No.Contains(kelime) || s.Aciklama.Contains(kelime))
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
                        serviceID = s.ServiceID,
        ...
        public List<ServisRepo> servisAraMRTip(string kelime, string tipID, int custID)
        {
            int tip = Int32.Parse(tipID);
            return (from s in db.services
                    where s.iptal == false && s.CustID == custID && s.tip_id == tip && (s.Baslik.Contains(kelime) || s.urun.Cinsi.Contains(kelime) || s.urun.imeino.Contains(kelime) || s.urun.serino.Contains(kelime) || s.urun.digerno.Contains(kelime) ||
                    s.Servis_Kimlik_No.Contains(kelime) || s.Aciklama.Contains(kelime))
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
        ...
        public List<ServisRepo> servisAraAcikKullaniciR(string kelime, string kullaniciID)
        {
            return (from s in db.services
                    where s.iptal == false && s.KapanmaZamani == null && (s.SonAtananID == kullaniciID || s.SonAtananID == "0") && (s.customer.Ad.Contains(kelime) || s.Baslik.Contains(kelime) || s.urun.Cinsi.Contains(kelime) || s.urun.imeino.Contains(kelime) || s.urun.serino.Contains(kelime) || s.urun.digerno.Contains(kelime) ||
                    s.Servis_Kimlik_No.Contains(kelime) || s.Aciklama.Contains(kelime) || s.customer.Adres.Contains(kelime) || s.customer.tanimlayici.Contains(kelime))
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
                        serviceID = s.ServiceID,
        ...
        public List<ServisRepo> servisAraAcikKullaniciRTip(string kelime, string kullaniciID, string tipID)
        {
            int tip = Int32.Parse(tipID);
            return (from s in db.services
                    where s.iptal == false && s.KapanmaZamani == null && (s.SonAtananID == kullaniciID || s.SonAtananID == "0") &&
                     s.tip_id == tip && (s.customer.Ad.Contains(kelime) || s.Baslik.Contains(kelime) || s.urun.Cinsi.Contains(kelime) || s.urun.imeino.Contains(kelime) || s.urun.serino.Contains(kelime) || s.urun.digerno.Contains(kelime) ||
                    s.Servis_Kimlik_No.Contains(kelime) || s.Aciklama.Contains(kelime) || s.customer.Adres.Contains(kelime) || s.customer.tanimlayici.Contains(kelime))
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
        ...
        public List<ServisRepo> servisAraKullaniciMR(string kelime, string kullaniciID, int custID)
        {
            return (from s in db.services
                    where s.CustID == custID && s.iptal == false && (s.Baslik.Contains(kelime) || s.urun.Cinsi.Contains(kelime) || s.urun.imeino.Contains(kelime) || s.urun.serino.Contains(kelime) || s.urun.digerno.Contains(kelime) ||
                    s.Servis_Kimlik_No.Contains(kelime) || s.Aciklama.Contains(kelime)) && (s.SonAtananID == kullaniciID || s.SonAtananID == "0")
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
                        serviceID = s.ServiceID,
        ...
        public List<ServisRepo> servisAraKullaniciMRTip(string kelime, string kullaniciID, string tipID, int custID)
        {
            int tip = Int32.Parse(tipID);
            return (from s in db.services
                    where s.CustID == custID && s.iptal == false && s.tip_id == tip && (s.Baslik.Contains(kelime) || s.urun.Cinsi.Contains(kelime) || s.urun.imeino.Contains(kelime) || s.urun.serino.Contains(kelime) || s.urun.digerno.Contains(kelime) ||
                    s.Servis_Kimlik_No.Contains(kelime) || s.Aciklama.Contains(kelime)) && (s.SonAtananID == kullaniciID || s.SonAtananID == "0")
                    orderby s.AcilmaZamani descending
                    select new ServisRepo
                    {
