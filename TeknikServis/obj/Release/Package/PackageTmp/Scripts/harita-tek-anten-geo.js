                ...
                if (!center_Lat) {
                    ...
                    geocoder.geocode({ 'address': musteri_adres }, function (results, status) {
                        if (status == google.maps.GeocoderStatus.OK) {
                            ...
                            musteriListesi[musteri_id] = { center: new google.maps.LatLng(results[0].geometry.location.lat(), results[0].geometry.location.lng()), adi: musteri_adi, musteri_id: musteri_id, musteri_adres: musteri_adres, lat: results[0].geometry.location.lat(), lng: results[0].geometry.location.lng() };
                    ...
                    musteriListesi[musteri_id] = { center: new google.maps.LatLng(center_Lat, center_Long), adi: musteri_adi, musteri_id: musteri_id, musteri_adres: musteri_adres, lat: center_Lat, lng: center_Long };
...
function goster() {
    ...
    for (var musteri in musteriler) {
        var nokta = {
            "id": musteriler[musteri].musteri_id.toString(),
            "lat": musteriler[musteri].lat.toString(),
            "lng": musteriler[musteri].lng.toString()
        };

        queryStr = JSON.stringify(nokta);
        noktalar.push(queryStr);
    }
...
function MusteriUpdate() {
    ...
    //console.log(JSON.stringify(noktalar));

    var bilgiler = "{ bilgiler:" + noktalar + "}";
    console.log("gönderdiğimiz");
    console.log(bilgiler);

    $.ajax({
        type: 'POST',
        url: '/TeknikTeknik/ResimKaydet.aspx/MusteriUpdate',
        ...
        beforeSend: function () {
            ...
            //console.log('noktamiz' + noktalar);
            //console.log('musteriler' + musteriler);
        },
        complete: function () {
        ...
        success: function (response) {
            ...
            console.log('tamamlandı' + response.d);
        }
    });
...
" </div>");
            ...
            document.getElementById("txtNoktalar").value += musteriler[musteri].musteri_id + "-" + musteriler[musteri].lat + "-" + musteriler[musteri].lng + ",";
        }
    };
